...
// TAKEN FROM THE DECOMPOSE LIBRARY. It is useful on android still.
/**
...
private fun Modifier.handleBackGestures(
    ...
    pointerInput(backDispatcher, leftEdgeEnabled, rightEdgeEnabled) {
        awaitEachGesture {
            ...
            val edge =
                when {
                    isLeftEdge && isRightEdge -> if (startPosition.x < size.width / 2F) BackGestureHandler.Edge.LEFT else BackGestureHandler.Edge.RIGHT
...
private fun BackGestureHandler.Edge.toSwipeEdge(): SwipeEdge =
    when (this) {
        BackGestureHandler.Edge.LEFT -> com.arkivanov.essenty.backhandler.BackEvent.SwipeEdge.LEFT
        BackGestureHandler.Edge.RIGHT -> com.arkivanov.essenty.backhandler.BackEvent.SwipeEdge.RIGHT
    }
